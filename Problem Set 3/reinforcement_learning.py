    ...
    def act(self, env: Environment[S, A], observation: S, training: bool = False) -> A:
        ...
        if training and self.should_explore():
            ...
            for action in actions:
                if self.compute_q(env, observation, action) > best_q:
                    best_q = self.compute_q(env, observation, action)
                    best_action = action
            return best_action
...
class SARSALearningAgent(RLAgent[S, A]):
    ...
    def update(
        ...
        if next_action is None:
            next_action = 0
            self.Q[state][action] = self.compute_q(
                env, state, action
            ) + self.learning_rate * (reward - self.Q[state][action])
        else:
            self.Q[state][action] = self.compute_q(
                env, state, action
            ) + self.learning_rate * (
                reward
                + self.discount_factor * self.compute_q(env, next_state, next_action)
                - self.Q[state][action]
            )
        ...
        with open(file_path, "w") as f:
            Q = {
                env.format_state(state): {
                    env.format_action(action): value
    ...
    def compute_utility(self, env: Environment[S, A], state: S) -> float:
        ...
        for action in actions:
            ...
            if q > max_q:
                ...
                max_state = state
                max_action = action
        return self.compute_q(env, max_state, max_action)
    ...
    def update(
        ...
        if done:
            ...
            self.Q[state][action] = self.compute_q(
                ...
                * max(self.compute_q(env, next_state, a) for a in env.actions())
                - self.Q[state][action]
            )
...
class ApproximateQLearningAgent(RLAgent[S, A]):
    ...
    def __compute_utility_from_features(self, features: Dict[str, float]) -> float:
        ...
        for action in self.actions:
            max_q = max(max_q, self.__compute_q_from_features(features, action))
        return max_q
    ...
    def save(self, env: Environment[S, A], file_path: str):
        with open(file_path, "w") as f:
            weights = {
                env.format_action(action): w for action, w in self.weights.items()
            }
    ...
    def load(self, env: Environment[S, A], file_path: str):
        with open(file_path, "r") as f:
            weights = json.load(f)
            self.weights = {
                env.parse_action(action): w for action, w in weights.items()
            }
